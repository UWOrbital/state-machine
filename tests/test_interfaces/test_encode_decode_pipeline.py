...
if len(os.listdir(Path("backend/interfaces/").resolve())) != 0:
    ...
    def test_receive():
        ...
        data_to_decode = fec_coder.decode(
            bin[INFO_FIELD_START_POSITION : INFO_FIELD_END_POSITION + 1]
        )
        # With the data decoded we need to add the rest of the data back to get a full frame
        decoded_data = bytes(
            bin[:INFO_FIELD_START_POSITION]
            + data_to_decode
            + bin[INFO_FIELD_END_POSITION + 1 :]
        )
    ...
    def test_uFrame_receive():
        ...
        assert rcv_frame.control.poll_final
    ...
    def test_command_send():
        ...
        data = bytes(cmd_ping_packed + cmd_rtc_sync_packed).ljust(
    ...
    def test_receive_command():
        ...
        match_cmd = [
            CmdCallbackId.CMD_EXEC_OBC_RESET.value,
            CmdCallbackId.CMD_PING.value,
        ]
