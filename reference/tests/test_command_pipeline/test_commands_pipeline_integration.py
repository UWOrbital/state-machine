...
def test_build_queue_integration(
    db_session: Session, pipeline: CommandsPipeline, comms_session: CommsSession
):
    ...
    mc_high = MainCommand(
        id=3, name="DownlinkLogs", data_size=1, total_size=1, priority=10
    )
    ...
    cmd_low = Command(
        type_=mc_low.id,
        params="rtc_time,12345678",
        status=CommandStatus.PENDING,
        session_id=comms_session.id,
    )
    cmd_high = Command(
        type_=mc_high.id,
        params="log_level,1,time_of_execution,0",
        status=CommandStatus.PENDING,
        session_id=comms_session.id,
    )
...
def test_queue_to_packet_integration(
    db_session: Session,
    ...
    monkeypatch.setattr(
        "data.resources.commands_pipeline.command_multi_pack",
        lambda x: [b"packet_data"],
    )
    monkeypatch.setattr(
        "data.resources.commands_pipeline.CommandPackaging.encode_frame",
        lambda self, x: b"FRAME_" + x,
    )
