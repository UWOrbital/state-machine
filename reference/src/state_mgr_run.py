from reference.src.database.crud import (
    get_next_session,
    update_current_session_status,
    get_all_commands_next_session,
    update_command_response,
    get_main_command_by_id,
    update_command_status,
    create_telemetry,
    clear_telemetry_by_type,
)
from reference.src.enums.command_enums import SessionStatus, CommandStatus
from reference.src.uart_connection import UartConnection
...
def main():
    ...
    while 1:
        ...
        # Wait for session
        time_next_session_sec = (
            next_session - datetime.now(timezone.utc)
        ).total_seconds()
        ...
        reset_command = "--command CMD_EXEC_OBC_RESET"  # noqa: F841
