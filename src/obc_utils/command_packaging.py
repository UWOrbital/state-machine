...
class CommandPackaging:
    ...
    def encode_frame(self, data: None | bytes) -> bytes:
        ...
        if data is None:
            ...
            if len(data) > RS_DECODED_DATA_SIZE:
                raise ValueError(
                    f"Invalid Argument: Data to encode is too long, must <= {RS_DECODED_DATA_SIZE} bytes"
                )
            # Encrypt data
    ...
    def decode_frame(self, data: bytes) -> Frame | None:
        ...
        if len(data) == 0:
            ...
            fec_data = self._fec.decode(
                data_unstuffed[INFO_FIELD_START_POSITION : INFO_FIELD_END_POSITION + 1]
            )
            # With the data decoded we need to add the rest of the data back to get a full frame
            decoded_data = bytes(
                data_unstuffed[:INFO_FIELD_START_POSITION]
                + fec_data
                + data_unstuffed[INFO_FIELD_END_POSITION + 1 :]
            )
