...
def send_command(
    args: str, com_port: str, timeout: int = 0
) -> CmdRes | type[CmdRes] | None:
    """
    ...
    send_bytes = comms.encode_frame(command_multi_pack(data)[0]).ljust(
        _PADDING_REQUIRED, b"\x00"
    )
    # Initialize pyserial
    with Serial(
        com_port,
        ...
        if command.id == CmdCallbackId.CMD_EXEC_OBC_RESET.value:
            print(read_bytes)
            return CmdRes(
                CmdCallbackId.CMD_EXEC_OBC_RESET,
                CmdResponseErrorCode.CMD_RESPONSE_SUCCESS,
                0,
            )
...
def parse_cmd_rtc_time_sync() -> ArgumentParser:
    ...
    parser = ArgumentParser(
        parents=[parent_parser], add_help=False, exit_on_error=False
    )
    parser.add_argument(
        "-rtc",
        "--rtc_sync_time",
...
def generate_command(args: str) -> tuple[CmdMsg | None, bool]:
    ...
    for func in child_parsers:
        ...
        if hasattr(command_args, "arg3"):
            ...
            command = commmand_factories[command_enum.value](
                command_args.arg1, command_args.timestamp
            )
        ...
        while True:
            ...
            if start_index != -1:
                ...
                if rcv_frame is not None and rcv_frame.data is not None:
                    command_res = parse_command_response(
                        bytes(rcv_frame.data[:RS_DECODED_DATA_SIZE])
                    )

                data_string = (
                    outer_bytes_left.decode("utf-8")
                    + str(command_res)
                    + outer_bytes_right.decode("utf-8")
                )
